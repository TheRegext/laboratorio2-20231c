...
void ListaDeTareas::mostrar(bool incluirDeadline) {
  Fecha fechaActual(20, 4, 2023);
  cout << "------------------------------------------------------------" << endl;
  ...
  for (int i = 0; i < _cantidad; i++) {
    ...
    if (incluirDeadline)
    {
        cout << setw(8) << setiosflags(ios::left) << diferenciaDias(fechaActual, _listaTareas[i].getFechaLimite());
    }
